...
class CKEngine:
    ...
    async def run_init(self) -> None:
        ...
        try:
            ...
            logger.exception("词库初始化(#INITROBOT#)执行异常")
    ...
    async def handle(self, ctx: Ctx) -> bool:
        ...
        try:
            ...
            logger.exception("词库块执行异常 (trigger=%s, source=%s:%s)",
                             blk.trigger, blk.source, blk.lineno)
    ...
    async def _delayed_call(self, delay: float, command: str, ctx: Ctx, depth: int) -> None:
        ...
        try:
            ...
            logger.exception("$调用$ 延迟指令异常 (command=%s)", command)
            return
        if sub.send:
            try:
                await sub.send(sub)
            except Exception:
                logger.exception("$调用$ 延迟指令输出发送失败 (command=%s)", command)
